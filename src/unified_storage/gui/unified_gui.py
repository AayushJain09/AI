...
from unified_storage.preprocessing.input_manager import (
...
class UnifiedGUI:
    ...
    def __init__(self):
        ...
        # Initialize unified storage system (replaces separate components)
        self.unified_store = UnifiedStore(data_dir="data")
        
        # Extract components for backward compatibility
        self.input_manager = None  # Will use unified store directly
        self.persistence = None    # Will use unified store directly 
        self.chromadb_indexer = self.unified_store.chromadb_indexer
    ...
    def setup_add_item_tab(self):
        ...
        # Processing options
        options_frame = ttk.LabelFrame(left_panel, text="Processing Options", padding="10")
        options_frame.pack(fill=tk.X, pady=(0, 20))
        
        # Augmentations per image
        ttk.Label(options_frame, text="Augmentations per image:").pack(anchor=tk.W)
        self.aug_count_var = tk.IntVar(value=50)  # Default to existing system value
        aug_count_spinbox = ttk.Spinbox(
            options_frame, 
            from_=1, 
            to=100, 
            textvariable=self.aug_count_var,
            width=10
        )
        aug_count_spinbox.pack(anchor=tk.W, pady=(0, 5))
        
        # Quality level
        ttk.Label(options_frame, text="JPEG Quality:").pack(anchor=tk.W)
        self.quality_var = tk.IntVar(value=95)  # Default to existing system value
        ...
        quality_spinbox.pack(anchor=tk.W, pady=(0, 10))
        ...
        def process_item():
            try:
                ...
                for i, image_path in enumerate(valid_images):
                    ...
                    # Store image with unified storage (includes feature extraction and ChromaDB indexing)
                    image_id = self.unified_store.store_image(
                        image_path=image_path,
                        image_id=f"{item_info.item_id}_{i}",
                ...
                # Create mock save result for compatibility
                class MockSaveResult:
                    def __init__(self):
                        self.success = True
                        self.augmented_count = stored_count  # No augmentation in unified store
                        self.feature_count = stored_count    # 1 feature per image
                        self.storage_size_mb = 0.0           # Not calculated
                        self.processing_time = time.time() - start_time
                        self.index_updated = True
                        self.error_message = None
                
                save_result = MockSaveResult()
                
                if save_result.success:
                    progress_window.complete_processing(
                        True,
                        f"✅ Item '{item_info.item_name}' added successfully!\n"
                        ...
                        f"Processing time: {save_result.processing_time:.2f} seconds\n"
                        f"ChromaDB index updated: {'✅' if save_result.index_updated else '❌'}"
                    )
    ...
    def run_recognition_test(self):
        ...
        if not self.chromadb_indexer.current_index:
            # Try to load index
            if not self.chromadb_indexer.load_index_from_database():
                messagebox.showerror("Error", "No ChromaDB index available. Please add some items first.")
                return
        
        try:
            # Use unified storage for search (includes feature extraction and ChromaDB search)
    ...
    def update_statistics(self):
        ...
        try:
            ...
            chromadb_stats = self.chromadb_indexer.get_statistics()
            ...
            # ChromaDB statistics
            current_index = chromadb_stats.get('current_index', {})
            ...
            stats_text += f"  Searches Performed: {chromadb_stats.get('searches_performed', 0)}\n"
            stats_text += f"  Average Search Time: {chromadb_stats.get('average_search_time', 0.0):.4f}s\n\n"
            
            # System performance
            stats_text += "🚀 SYSTEM PERFORMANCE:\n"
            stats_text += f"  Indices Built: {chromadb_stats.get('indices_built', 0)}\n"
            stats_text += f"  Average Build Time: {chromadb_stats.get('average_build_time', 0.0):.2f}s\n"
            stats_text += f"  Vectors Added: {chromadb_stats.get('vectors_added', 0)}\n"
            stats_text += f"  Total Build Time: {chromadb_stats.get('total_build_time', 0.0):.2f}s\n\n"
