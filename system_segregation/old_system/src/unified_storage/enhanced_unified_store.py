...
- ✅ ChromaDB vector indexing for scalable search
...
- Integrates with unified storage backend (ChromaDB + SQLite)
...
class AugmentationConfig:
    ...
    """
    # Core settings from your original system
    augmentations_per_image: int = 30  # Your proven value for high accuracy
    target_size: Tuple[int, int] = (1024, 1024)
    quality: int = 85  # Optimized for storage vs quality balance
    
    # Your proven strategy weights (sum to 1.0)
...
class EnhancedUnifiedStore(UnifiedStore):
    ...
    def _create_texture_background(self) -> np.ndarray:
        ...
        h, w = self.augmentation_config.target_size
        background = np.random.randint(0, 255, (h, w, 3), dtype=np.uint8)
